...
public class GamingUI : MonoBehaviour
{
    ...
    public void Show_or_hideGameUI(bool isshow)
    {
        TimeText.enabled = isshow;
        ProgressText.enabled = isshow;
    }
    ...
    public void UpDateScoreBoard()
    {
       ...
       var Players = PhotonNetwork.PlayerList;
        Array.Sort(Players, (p1, p2) => (int)(p2.GetScore() - p1.GetScore()));
        foreach(var player in Players)
        {
            builder.AppendLine($"{player.NickName} : {player.GetScore().ToString("f2")}%");
