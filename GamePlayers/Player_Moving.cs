...
/// <summary>
/// プレイヤーの操作を反映したクラス。ここをオフにすれば操作不可能になる。ここで個人個人に対する設定もしておく
/// </summary>
public sealed class Player_Moving : MonoBehaviourPunCallbacks
{
    ...
    void Start()
    {
        ...
        Isretrying = false;
        countretry = 0;
        UserManager.Show_or_HideVisualHook(this , photonView.IsMine);
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.B) && !GameManager.instance.IsTutorial)
        {
            if (!Isretrying)
            {
                Isretrying = true;
                GameManager.instance.SetGotitle();
            }
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision) //チュートリアル終わり
    {
