...
public class FookGenerator : MonoBehaviourPunCallbacks
{
    ...
    private SpriteRenderer MoucePointer; //マウス追従のSprite
    ...
    private void Start()
    {
        ...
        MoucePointer = GameObject.Find("MoucePointer").GetComponent<SpriteRenderer>();
        hinge.enabled = false;
    ...
    private bool CheckCanShootFook3()  //フックが射出可ならtrue、そうでなければfalse。色判定用に、フック中かどうかの判断は含んでいない
    {
        ...
        moucepos.z = 0f;
        Vector3 worldpos = Camera.main.ScreenToWorldPoint(moucepos);
        RaycastHit2D hitinfo = Physics2D.Raycast(transform.position, worldpos - transform.position, fooklinelength * 25f, StageLayer | BlockLayer);
        if(!hitinfo.collider)
        {
            return false;
        }
        ...
        {
            LastFookNumber = (int)(distance / fooklinelength);
            return true;
        }
        return false;
    }
